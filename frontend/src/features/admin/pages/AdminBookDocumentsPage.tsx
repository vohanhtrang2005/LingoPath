...
import type { BookDocument, ExtractedPageText } from "../../../types/content";

export function AdminBookDocumentsPage() {
  ...
  const loadPages = useCallback(async (document: BookDocument) => {
    ...
    setPagesError(null);
    try {
      const response = await contentApi.getExtractedPages(document.id);
