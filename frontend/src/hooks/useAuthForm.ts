...
async function postAuth(path: '/login' | '/register', body: { email: string; password: string }) {
  ...
  const payload = await response.json() as AuthResponseBody;
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      const authData = mode === 'signup'
        ? await postAuth('/register', credentials).then(() => postAuth('/login', credentials))
        : await postAuth('/login', credentials);
